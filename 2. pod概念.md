...
- 删除deployment或者其他controller被删除；
- 更新deployment上的pod template 导致的pod重启；
- 直接删除pod；
- node节点的修复和更新；
- node节点的扩容和收缩；
- 把pod移动到一个更适合的node节点上；

### 如何避免突发状况

- 确认集群有足够的资源运行；
- 配置pod的副本数量满足高可用；
- 为了更强的高可用要求，可以使用anti-affinity 跨机柜跨区域部署pod副本；

### DPB的工作原理 PodDisruptionBudget

​		可以给应用配置一个`PodDisruptionBudget` 对象。PDB限制了pod在自发终止voluntary disruptions的时候允许终止的副本数量。

​		滚动升级rolling update的操作引起的pod的删除终止并不会受限于PDB，当rolling update的时候，可以同时终止的pod的数量的配置在controller spec中配置
